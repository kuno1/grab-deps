...
function generateHandleName( filePath, srcDir, namespace ) {
	...
	// Convert path separators to dashes and remove leading file type if it matches
	const pathParts = pathWithoutExt.split( path.sep );

	// Remove the file type from the beginning of the path if it exists
	// For example: 'js/components/pagination' -> 'components/pagination' for JS files
	if ( pathParts[ 0 ] === fileType ) {
		pathParts.shift();
	}

	const handleName = `${ namespace }-${ fileType }-${ pathParts.join(
		'-'
	) }`;
